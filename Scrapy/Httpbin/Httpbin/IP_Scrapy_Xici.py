...
class GetIP(object):
    ...
    def judge_ip(self,http_type,ip,port):
        ...
        try:
            proxy = {http_type.casefold(): proxy_url}
            response = requests.get(http_url,proxies=proxy,headers=self.headers)
        ...
        if code == 200:##code >= 200 and code < 300
            # print('effective ip')
            return True
        else:
            print("Invalid ip and port")
    ...
    def get_random_ip(self):
        ...
        for ip_info in cursor.fetchall():
            ...
            if judge_re:
                return http_type.casefold()+"://"+ip+":"+port
            else:
                sleep(0.001)
